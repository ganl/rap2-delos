    ...
    // (async () => {
      for (let i = 0; i < properties.length; i++) {
        ...
        if (item.type === 'Array' || item.type === 'Object') {
          let subObject = (item.type === 'Array' && item.children.length) ? (jsonObject[item.name] || [{}])[0] : (jsonObject[item.name] || {})
          ...
          let element = {
            'name': item.name,
            'old': item.value,
            'new': jsonObject[item.name] || "''|undefined",
            'updated': false,
            'skiped': false
          }
          if (jsonObject[item.name] !== '' && typeof jsonObject[item.name] !== 'undefined' && item.value != jsonObject[item.name]) {
...
router.post('/spec/request/params/update', async (ctx) => {
  ...
  if (itfId) {
    ...
    for (let i = 0; i < scopes.length; i++) {
      let updatedProperties = await Property.findAll({
        attributes: { exclude: [] },
        where: { interfaceId: itf.id, scope: scopes[i] },
      })
